...
2026-09-21 보완: **현재 FOTA만 가능한 조건에서 공용·부서별 First를 선택하는 비교**는 [RV-001](reviews/001-first-ica-structure.md#device-fota)에 기록한다. 아래 독립 업데이트 권한·신뢰 번들은 향후 도입 제안이며 기존 단말이 이미 지원하는 기능이 아니다. First의 단말별 고정·신뢰 범위도 아직 확인해야 한다.

같은 날 추가한 [Root만 초기 탑재하고 서버가 First를 관리하는 후보](reviews/001-first-ica-structure.md#root-only-server)는 인증서 배포·인증된 목록 배포·서버 검증 위임을 구분한다. Root로 목록 관리자를 인증하는 것과 아래의 독립 U는 동일한 복구 보장이 아니다. 해당 Root가 침해돼도 안전한 갱신을 보장하려면 별도 복구 신뢰 근거를 확인해야 한다. 두 후보 모두 미확정이다.
